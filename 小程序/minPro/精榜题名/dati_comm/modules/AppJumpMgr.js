...
class ADLayer {
  ...
  constructor(moshi, wzNode, clickMask) {
    ...
    for (var i = 0; i < adList.length; i++) {
      ...
      if (moshi == 3)
      {
        var saveidattr = info.getAttribute('saveid')
        if (saveidattr) saveid = parseInt(saveidattr) 
      }

      var item = {
        sid: parseInt(info.getAttribute('id')),
        saveid: saveid,
        qz: parseInt(info.getAttribute('qz')),
        path: info.getAttribute('path'),
        appId: info.getAttribute('appId'),
        picture: Player.ArticleServerUrl().MergePath(  "/public/uploads/ProblemImg/").MergePath(  info.getAttribute('pic')),
  ...
  {
     var list = []
    if (this.adList.length < 1) return list;
...
function Random(wz) {
  ...
  for (var i =1; i <= 3; i++) {
    ...
    if(i==3) //保底换量位，需要支持同时显示多个
    {
      var jumpList = layer.List()
      g_CurrJumpList[wz] = jumpList

      if(jumpList.length>0)
      {
         g_CurrJump[wz] = jumpList[0];
         break
      }
    }
    else
    { 
      ...
      if (jumpObj) {
        ...
        g_CurrJumpList[wz].push(jumpObj)
        break
      }
    }
  }
...
//获取当前跳转列表
function CurrList(wz) {
  var re={}
   re= g_CurrJumpList[wz] 
  //  console.log("rereere",re)
  return re//数组
}

function _Jump(jumpObj,wz, cb)
{
  var jumpID = jumpObj.saveid
  var moshi = jumpObj.moshi
...
//执行跳转行为，从CurrList中选择一个
function JumpList(wz,idx, cb){
  if(!g_CurrJumpList[wz]||g_CurrJumpList[wz].length<1 ) return//不存在跳转
  if(idx<0||idx>=g_CurrJumpList[wz].length) return//非法的索引
  var jumpObj = g_CurrJumpList[wz][idx]
  _Jump(jumpObj,wz, cb||null)
}
...
module.exports = { 
  Random, Curr, Jump, Jumpdati, OnJumpAppSuccess, GetClickNum,
  JumpList,CurrList
}